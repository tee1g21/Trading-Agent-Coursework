...
# ------------------------[COMPANY]------------------------
class SomewhatCoolCompany(TradingCompany):
    ...
    def __init(self) -> None:
        ...
        self.default_company_profit_factor = 1.65
    ...
    def receive(self, contracts, auction_ledger=None, *args, **kwargs):
        ...
        for company in self.headquarters.get_companies():
           if company.name not in self.company_profit_factors:
               self.company_profit_factors[company.name] = [self.default_company_profit_factor]
        ...
        for company_name, won_contracts in auction_ledger.items():
            ...
            for contract in won_contracts:
                ...
                # Predict payment using the current profit factor
                ...
                for vessel in company_fleet:
                    predicted_cost = self.trade_cost_with_reloc(vessel, trade)
                ...
                # Clamp the profit factor between 1 and 2
                clamped_factor = max(1, min(profit_factor, 2))
                self.company_profit_factors[company_name].append(clamped_factor)
    ...
    def predict_payment(self, company_name, trade):
        ...
        average_profit_factor = self.average_profit_factor(company_name)
    ...
    def average_profit_factor(self, company_name):
        # Calculate the average profit factor for the company across all trades
        if len(self.company_profit_factors[company_name]) != 0:
            average_profit_factor = sum(self.company_profit_factors[company_name]) / len(self.company_profit_factors[company_name])
        else:
            average_profit_factor = self.default_company_profit_factor    
        return average_profit_factor
    ...
    def cost_to_closest_future_trade(self, vessel: VesselWithEngine, trade: Trade) -> float:
        ...
        if self._future_trades:
            ...
            # for each future trade calculate the distance from the current trade destination
    ...
    def predict_competitor_bid(
        ...
        """
        average_profit_factor = self.average_profit_factor(owner.name)
    ...
    def trade_cost_with_reloc(
        ...
        return (self.trade_cost(vessel, trade)
                + self.p2p_cost(SomewhatCoolCompany.get_end_location(vessel), trade.destination_port, vessel))
