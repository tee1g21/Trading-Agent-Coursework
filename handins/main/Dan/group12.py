...
from mable.shipping_market import Contract, Trade
...
class Company12(TradingCompany):
    ...
    def _schedule(
        ...
        for vessel, current_schedule in schedules.items():
            for schedule in Company12.generate_options(current_contract.trade, current_schedule):
...
# ----------------[UTILITY FUNCTIONS]-------------------
    ...
    def trade_cost_with_reloc(
        ...
        return (self.trade_cost(vessel, trade)
                + self.p2p_cost(Company12.get_end_location(vessel), trade.destination_port, vessel, False))
    ...
    def update_competitor_info(
        ...
        for company_name, won_contracts in auction_ledger.items():
            ...
            profit_factor_avg = Company12.clamp(1.4, profit_factor_sum / len(won_contracts), 3)
